    ...
    for idx, row in df_raw_data.iterrows():
        ...
        for sensor in sensors:
            ...
            query = "INSERT INTO DULLNDGGDT(OBSDT, DAMCD, SENID, MEAVAL1, CALVAL1, CALVAL2, CALVAL3, CALVAL4) " \
                ...
                sensor['DAMCD'],
                sensor['SENID'],
                sensor['M01'],                  # 측정값 (M01 - 0) * 1 + 0
                sensor['C01'],                  # 기본계산식 적용 결과
                sensor['C02'],                  # 온도보정값 (0)
                sensor['C03'],                  # 온도보정결과
                round(sensor['C04'], 8),        # 단위보정결과 (소수점 여덟째자리 반올림)
            )
            ...
            try:
                ...
                # sqlstate = ex.args[0]
                logging.error('[pyodbc Exception] sqlstate:{0}'.format(ex))
                continue
    ...
    for idx, row in df_raw_data.iterrows():
        ...
        for sensor in sensors:
            ...
            query = "INSERT INTO DULLNDGGDT(OBSDT, DAMCD, SENID, MEAVAL1, MEAVAL2, CALVAL1, CALVAL2, CALVAL3, " \
                ...
                sensor['C03'],
                sensor['C04'],
                round(sensor['C05'], 8)     # 소수점 8자리 반올림
            )
...
def get_db_connection(cfg):
    provider1 = 'DRIVER={{Tibero 6 ODBC Driver}};SERVER={0};PORT={1};UID={2};PWD={3};DATABASE={4}'.format(
        cfg['DB1']['IP'],
        cfg['DB1']['PORT'],
        cfg['DB1']['UID'],
        cfg['DB1']['PWD'],
    ...
    provider2 = 'DRIVER={{Tibero 6 ODBC Driver}};SERVER={0};PORT={1};UID={2};PWD={3};DATABASE={4}'.format(
        cfg['DB2']['IP'],
        cfg['DB2']['PORT'],
        cfg['DB2']['UID'],
        cfg['DB2']['PWD'],
